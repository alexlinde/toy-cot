...
A rationale is a plain string and every one of its tokens is supervised: the
whole pipeline trains on gold traces, whether they come from the generator or
from a sampled chain that reproduced one (see onpolicy.py).
"""
...
class TextProcessor:
    ...
    def prepare_input_sequence(
        ...
        `rationale` is a plain string (the empty string / None gives an empty
        <THINK> span, the no-CoT ablation); every rationale token is supervised.
        """
        tok = self.tokenizer

        q_tokens = tok.tokenize(question)
        a_tokens = tok.tokenize(answer) if answer is not None else []
        r_tokens = tok.tokenize(rationale) if rationale else []
        ...
        assert th_e - th_s - 1 == len(r_tokens), (
            ...
            f"the rationale tokenized to {len(r_tokens)}"
        )
        assert fn_s > th_e and fn_e > fn_s, "[text] Invalid FINAL span"
        assert eos_p == len(input_ids)-1, "[text] <EOS> must be final token"

        # Supervision must land exactly on the rationale span: one supervised
        # target per rationale token, plus </THINK>.
        assert sum(rat_mask) == len(r_tokens) + 1, (
            f"[text] rat_mask supervises {sum(rat_mask)} positions, expected "
            f"{len(r_tokens) + 1} (rationale tokens + </THINK>)"
        )
