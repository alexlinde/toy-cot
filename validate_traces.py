...
def main() -> int:
    ...
    if unused:
        # 'wait' is a legacy vocabulary token kept for checkpoint compatibility
        # (see RationaleGenerator.vocabulary), so it is expected here forever.
        note = ("  ('wait' is a legacy token no generator emits; it stays in the "
                "vocabulary for checkpoint compatibility)"
                if unused == ['wait'] else "")
